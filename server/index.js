...
// Our middleare.
app.use(cors({ origin: process.env.NODE_ENV=='development' ?  'http://localhost:3000' : process.env.FRONTEND_ORIGIN, credentials: true }));
...
function Auth(req, res, next) {
  try {
    ...
    // verify the token
    const decodedToken = jwt.verify(token, process.env.JWT_SECRET || 'key', { expiresIn: '30d' });
...
app.get('/verify', async (req, res) => {
  try {
    ...
    // verify the token
    const decodeToken = jwt.verify(token, process.env.JWT_SECRET || 'key', { expiresIn: '30d' });
    const { user_email } = decodeToken;
    // if we have a valid token then we return true
    if (decodeToken && user_email && typeof user_email === 'string' && user_email.length > 0) {
...
privateRoutes.post('/todo', async (req, res) => {
  try {
    ...
    const newTodo = await pool.query('INSERT INTO todo (description, user_email) VALUES($1, $2) RETURNING *', [description, user_email]);
...
privateRoutes.get('/todo', async (req, res) => {
  try {
    ...
    const allTodos = await pool.query('SELECT * FROM todo WHERE user_email = $1', [user_email]);
...
privateRoutes.get('/todo/:id', async (req, res) => {
  try {
    ...
    const todo = await pool.query('SELECT * FROM todo WHERE todo_id = $1 AND user_email = $2', [id, user_email]);
...
privateRoutes.put('/todo/:id', async (req, res) => {
  try {
    ...
    const updateTodo = await pool.query('UPDATE todo SET description = $1 WHERE todo_id = $2 AND user_email = $3 RETURNING *', [description, id, user_email]);
...
privateRoutes.delete('/todo/:id', async (req, res) => {
  try {
    ...
    const deleteTodo = await pool.query('DELETE FROM todo WHERE todo_id = $1 AND user_email = $2 RETURNING *', [id, user_email]);
